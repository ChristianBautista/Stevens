...
void* fs_init(struct fuse_conn_info *conn)
{
	...
	if (disk->ops->read(disk, inode_map_base, 1, inode_map) < 0) exit(1);
...
{
	/**NOTE:
	 * The entirety of the code below is
	 * copied from fs_mknod.
	 * The only line that has to be changed is
	 * when calling set_attributes_and_update
	 */
	...
	if (disk->ops->read(disk, parent_inode->direct[0], 1, entries) < 0)
		exit(1);
	//assign inode and directory and update
	// This line is the only difference between this method and fs_mknod
	int res = set_attributes_and_update(entries, name, mode, true); // set isDir param to true since it is a dir
	if (res < 0) return res;
...
static int fs_rmdir(const char *path)
{
	...
	//get inodes and check
	//CS492: your code below
	// The code below is copied from fs_unlink as both methods take in *path
	char *_path = strdup(path);
    char name[FS_FILENAME_SIZE];
    int inode_idx = translate(_path);
    int parent_inode_idx = translate_1(_path, name);
	...
	//CS492: your code below
	// This code is also copied from fs_unlink as it performs the same task of removing an entry from a directory
	memset(entries, 0, DIRENTS_PER_BLK * sizeof(struct fs_dirent));
	if (disk->ops->read(disk, parent_inode->direct[0], 1, entries) < 0)
		exit(1);
	for (int i = 0; i < DIRENTS_PER_BLK; i++) {
...
{
	// CS492: your code below
	// This code is copied from fs_chmod as it performs the same task of retrieving the inode
	char* _path = strdup(path);
	int inode_idx = translate(_path);
	if (inode_idx < 0) return inode_idx;
	struct fs_inode *inode = &inodes[inode_idx];
	// use modtime to update the mtime param in inode
...
static void fs_read_blk(int blk_num, char *buf, size_t len, size_t offset) {
	...
	// This code is copied from fs_write modified to read, excluding the write line at the end
...
{
	//CS492: your code here
	// This code is copied from fs_write as it performs the same task of setting up the same variables
	char *_path = strdup(path);
    int inode_idx = translate(_path);
    if (inode_idx < 0) return inode_idx;
    struct fs_inode *inode = &inodes[inode_idx];
	...
	// adjust the offset based on what needs to be read
    if(offset+len > inode->size) {
        len = (size_t)(inode->size-offset);
    }

    size_t len_to_read = len; // the number of bytes to read

	// The code below loosely follows the code from fs_write
	// Although, it is much simpler since we are only reading the file
    //read direct blocks
