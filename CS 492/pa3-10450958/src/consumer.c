...
static int do_procs(int fd) {
	int i, status, ret = 0;
	pid_t pid;
	//char *buf = calloc(SCULL_IOCGETELEMSZ+1, sizeof(char));
	...
	for(i = 0; i < g_concurrency; i++) {
		...
		if(pid == 0) {
			...
			if((count = read(fd, buf, SCULL_IOCGETELEMSZ)) < 0)
                perror("read");
            if(count) {
                buf[SCULL_IOCGETELEMSZ] = '\0';  //set the '\0'
                printf("%s\n", buf);
            }
			/*
			//Use SCULL_IOCGETELEMSZ to get the size of an element and store it in num_bytes
			if(ioctl(fd, SCULL_IOCGETELEMSZ, &num_bytes) < 0)
				perror("ioctl");
			//read num_bytes from fd into buf and store in count while checking for error
			if(count = read(fd, buf, num_bytes))
				perror("read");
			//if read was successful, print buf
			if(count)
				printf("%s\n", *buf);
			*/
			/*
