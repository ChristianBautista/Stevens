...
do_join(ClientPID, ClientName, ClientID, Ref, ChatName) ->
    %% tell client we want to join chatroom with name ChatName. /join step 1
...
do_quit(ClientPID, Ref) ->
    %% tells client we want to quit. /quit step 1.
...
do_whoami(St, Ref, ClientPID) ->
    %% ask client what current nickname is. /whoami step 1
...
do_new_nick(St, Ref, ClientPID, Nick) ->
    %% tell client we would like to change nickname. /nick step 1.
...
%% Here, the GUI receives a message from the client process! Receveing client part 4
