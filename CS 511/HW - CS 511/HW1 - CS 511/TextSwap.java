...
public class TextSwap {
    ...
    private static Interval[] getIntervals(int numChunks, int chunkSize) {
        ...
        for(int i=0; i<numChunks; i++) {
            interval = new Interval(i*chunkSize,(i*chunkSize)+chunkSize-1);
    ...
    private static char[] runSwapper(String content, int chunkSize, int numChunks) {
        ...
        for(int i=0; i<newIntervals.length; i++) {
            newIntervals[i] = intervals[labels.get(i)-'a'];
        }
        char[] buffer = new char[content.length()];
        //Swapper swapper;
        Thread thread;
        for(int j=0; j<newIntervals.length; j++) {
            thread = new Thread(new Swapper(newIntervals[j], content, buffer, chunkSize*j));
            thread.start();
            /*
            swapper = new Swapper(newIntervals[j], content, buffer, chunkSize*j);
            swapper.run();
            */
        }
        return buffer;
