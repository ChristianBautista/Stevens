...
(define (compose RO RI)
  (if (or (null? RO) (null? RI)) '() (make-set (append (composeHelper RO (car RI)) (compose RO (cdr RI))))))

(define (composeHelper RO RI)
  (cond ((null? RO) '())
        ((eq? (car (cdr RI)) (car (car RO))) (cons (list (car RI) (car (cdr (car RO)))) (make-set (composeHelper (cdr RO) RI))))
...
(define (power R k)
  (cond ((or (null? R) (eq? k 0)) '())
        ((eq? k 1) R)
        (else (compose R (power R (- k 1))))))
...
(define (transitive-closure R)
  (if (null? R) '() (make-set (transitive-closurehelper R (cardinality R)))))
(define (transitive-closurehelper R k)
  (if (or (null? R) (eq? k 0)) '() (make-set (append (power R k) (transitive-closurehelper R (- k 1))))))
...
(define (transitive? R)
  (if (and (subset? R (transitive-closure R)) (subset? (transitive-closure R) R)) #t #f))
...
(define (EQ-relation? R n)
  (if (and (reflexive? R n) (and (symmetric? R) (transitive? R))) #t #f))
