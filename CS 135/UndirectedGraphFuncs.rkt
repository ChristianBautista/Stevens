...
(define (get-vertices G)
  (if (null? G) '() (usort (cons (caar G) (cons (cadar G) (get-vertices (cdr G)))))))
...
(define (all-even?-helper G L)
  (cond ((null? L) #t)
        ((odd? (degree G (car L))) #f)
        (else (all-even?-helper G (cdr L)))))
(define (all-even? G)
  (if (null? G) #t (all-even?-helper G (get-vertices G))))
...
(define (adjacent G v)
  (cond ((null? G) '())
        ((eq? (caar G) v) (usort (cons (cadar G) (adjacent (cdr G) v))))
        ((eq? (cadar G) v) (usort (cons (caar G) (adjacent (cdr G) v))))
...
(define (connected?-helper G V L)
  (cond ((null? L) #t)
        ((not (null? (remove V (usort (dfs G '() (car L)))))) #f)
        (else (connected?-helper G V (cdr L)))))
(define (connected? G)
  (if (null? G) #t (connected?-helper G (get-vertices G) (get-vertices G))))
