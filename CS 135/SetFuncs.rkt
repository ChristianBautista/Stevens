...
(define (set-equal? SA SB)
  (if (and (subset? SA SB) (subset? SB SA)) #t #f))
...
(define (cardinality L)
  (if (null? L) 0 (+ 1 (cardinality (cdr (make-set L))))))
...
(define (remove e S)
  (cond ((not (element? e S)) (make-set S))
        ((eq? e (car S)) (make-set (cdr S)))
        (else (make-set (cons (car S) (remove e (make-set (cdr S))))))))
