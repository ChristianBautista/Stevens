...
(define (euclid-gcd-helper r x y)
  (if (zero? r) x (euclid-gcd-helper (modulo y x) r y)))  
(define (euclid-gcd a b)
  (cond ((and (zero? a) (zero? b)) 0)
        ((zero? a) b)
        ((zero? b) a)
        ((< b a) (euclid-gcd b a))
        (else (euclid-gcd-helper (modulo b a) a b))))
...
(define (modulo-extractor cong-sys)
  (if (null? cong-sys) '() (cons (cadar cong-sys) (modulo-extractor (cdr cong-sys)))))
(define (pairwise-prime? x l)
  (cond ((null? l) #t)
        ((eq? (euclid-gcd x (car l)) 1) (pairwise-prime? x (cdr l)))
        (else #f)))
(define (CRT-exists?-helper mods)
  (cond ((null? mods) #t)
        ((eq? (pairwise-prime? (car mods) (cdr mods)) #f) #f)
        (else (CRT-exists?-helper (cdr mods)))))
(define (CRT-exists? cong-sys)
  (CRT-exists?-helper (modulo-extractor cong-sys)))
...
(define (CRT-helper cong-sys m)
  (if (null? cong-sys) 0 (+ (* (caar cong-sys) (* (/ m (cadar cong-sys)) (mul-inv (/ m (cadar cong-sys)) (cadar cong-sys)))) (CRT-helper (cdr cong-sys) m))))
