...
(define (encrypt plaintext key)
  (map (lambda (c) (mod-exp c (cadr key) (car key))) (map (lambda (x) (char->integer x)) (string->list plaintext))))
...
(define (decrypt ciphertext key)
  (list->string (map (lambda (x) (integer->char x)) (map (lambda (i) (mod-exp i (cadr key) (car key))) ciphertext))))
