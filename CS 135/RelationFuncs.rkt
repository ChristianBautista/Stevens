...
(define (id n)
  (if (eq? n 1) '((1 1)) (append (id (- n 1)) (list (cons n (list n))))))
...
;; Type Signature: (reflexive? relation int) -> boolean
(define (symmetric? R)
  (if (and (subset? R (make-set (append R (inverse R)))) (subset? (make-set (append R (inverse R))) R)) #t #f))
...
(define (relates-to v R)
  (cond ((null? R) '())
        ((eq? v (car (car R))) (cons (car (cdr (car R))) (relates-to v (cdr R))))
