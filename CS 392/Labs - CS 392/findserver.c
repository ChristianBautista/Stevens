...
int create_socket(struct timeval *tv) {
    ...
    //NOTE TO SELF: either AF_INET or AF_UNIX
    ...
    //NOTE TO SELF: maybe cast to (struct timeval *) instead of (char *)
    if (setsockopt(client_socket, SOL_SOCKET, SO_RCVTIMEO,(char *)&tv,sizeof(struct timeval)) == -1) {
...
//	puts("hii");
    ...
    //note: convert from little to big endian
...
//    	printf("i = %d.\n",i);
    	if (connect(client_socket, (struct sockaddr *)&serv_addr, addrlen) == 0) {
    		puts("connected.\n");
    		//note to self: success for recv might be == BUFLEN - 1
