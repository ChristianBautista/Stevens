...
void *sum_array(void *ptr) {
	thread_args thread_info = *(thread_args *)ptr;
	int partial_sum = 0;

	for(int i = thread_info.start_index; i < 2*thread_info.length; i+=2) {
		partial_sum += array[i];
	}
	...
	sum += partial_sum; /* this is the critical section */
	/* unlock the mutex */
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 2; i++) {
    	...
    	targs[i].length = (num_values%2==1) ? ((i==0) ? (num_values+1)/2 : (num_values)/2) : (num_values)/2;

		if ((retval = pthread_create(&threads[i], NULL, sum_array,
				(void*) (&targs[i]))) != 0) {
