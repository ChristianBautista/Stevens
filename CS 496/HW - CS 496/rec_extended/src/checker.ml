...
declaration")
  ...
  | Node(de, le, re) ->
    ...
    if d == l then
    if l == r then return (TreeType l) else error "node: node types do not match"
    else error "node: node types do not match"
  | NullT(t) ->
    type_of_expr t >>=
