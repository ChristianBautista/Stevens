...
let rec new_env : string list -> env ea_result = fun fs ->
  empty_env () >>+
  extend_env_list fs (List.rev (List.map (fun x -> (RefVal(Store.new_ref g_store (NumVal 0)))) (List.map (fun s -> StringVal s) fs)))
...
let lookup_method : string -> string -> class_env ->
  ...
  match (List.assoc_opt c_name c_env) with
  | Some (s,f,m_env) -> (List.assoc_opt m_name m_env)
  | None -> None
  )
...
and
  ...
  | NewObject(c_name,es) ->
    ...
    lookup_class c_name !g_class_env >>= fun triple -> (
      match triple with
      | (super,fields,methods) -> new_env fields >>= fun env -> (
        match (List.assoc_opt "initialize" methods) with
        | Some m -> apply_method "initialize" (ObjectVal(c_name,env)) args m >>= fun x ->
  ...
  | Send(e,m_name,es) ->
    eval_expr e >>= fun x -> (
      match x with
      | ObjectVal(c_name,c_env) -> sequence (List.map eval_expr es) >>= fun args -> (
        match lookup_method c_name m_name !g_class_env with
        | None -> error "Method not found"
        | Some m -> apply_method m_name (ObjectVal(c_name,c_env)) args m
        )
      | _ -> error "send: not an object"
    )
  | Self ->
    eval_expr (Var "_self")
  | Super(m_name,es) ->
