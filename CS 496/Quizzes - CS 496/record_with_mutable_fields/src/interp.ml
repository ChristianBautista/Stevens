(* 
Min Jee Cheon, Christian Bautista
Q6
4/14/21
...
and
  ...
  | Proj(e,id) ->
    (*is it mutable? if not look up standard way, recordval has the field
    if it is, the contents are a pointer to a memory address,
      fetch the address in memory
      look up in the store *)
    eval_expr e >>=
    fields_of_recordVal >>= fun v ->
    (match List.assoc_opt id v with
    | None -> error "error"
    | Some (false, e) -> return e
    | Some (true, RefVal l) -> Store.deref g_store l
  ...
  | SetField(e1,id,e2) ->
    ...
    | Some (true, _) -> error "error"
    | Some (false, e) -> error "does not have RefVal")
  | Unit -> return UnitVal
  | Debug(_e) ->
