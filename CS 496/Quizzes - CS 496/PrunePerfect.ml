...
(* Quiz 2 *)
(* 
   Christian Bautista and Julie Cheon
...
let rec prune n t =
  if (n>(height t)) then t else if n==0 then Empty else (
  let rec prune_rec c n t = 
    match t with
    | Node(d,lt,rt) when c!=n -> Node(d,(prune_rec (c+1) n lt),(prune_rec (c+1) n rt))
    | Node(d,lt,rt) when c==n -> Node(d,Empty,Empty)
    | Node(d,lt,Empty) when c==n -> Node(d,Empty,Empty) 
    | Node(d,Empty,rt) when c==n -> Node(d,Empty,Empty) in prune_rec 1 n t)
