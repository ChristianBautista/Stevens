...
let rec chk_expr : expr -> texpr tea_result = fun e ->
  ...
  | Proj(e,id) ->
    ...
    match r with
    | RecordType(fs) -> (
      let (ids,ts) = List.split fs in (
        let rec find i types = 
        match i with
        | [] -> error "proj: id not found"
        | h::t -> (
          match types with 
          | [] -> error "proj: incorrect length" 
          | a::b -> if h==id then return a else find types b)
        in find ids ts
      )      
    )
    | _ -> error "proj: argument is not a record"
    )
