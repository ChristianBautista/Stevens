...
public class RestMethod {
    ...
    protected ServerApi createClient(Uri serverUri, ChatServiceRequest request) {
         ...
         *Alternative: String.valueOf(serverUri)
                */
        Retrofit retrofit = new Retrofit.Builder().baseUrl(serverUri.toString()).addConverterFactory(GsonConverterFactory.create(gson)).client(client).build();
    ...
    public StreamingResponse perform(SynchronizeRequest request, final StreamingOutput out) throws IOException {
        ...
        // execute the Web service call
        callResponse = server.syncMessages(senderId, request.lastSequenceNumber, requestBody).execute();
        response = new ChatServiceResponse(callResponse) {
            @Override
...
//        //"X-Response-Message"
//        response.responseMessage = callResponse.headers().get("X-Response-Message");
//        response.httpResponseCode = callResponse.code();
//        response.httpResponseMessage = callResponse.message();
        // end
        /*
