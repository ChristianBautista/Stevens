...
class Term:
    ...
    def __neg__(self): # otherTerm = -myTerm
        ...
        newSelf = Term()
        newSelf.coef = -1*self.coef
        newSelf.exp = self.exp
        return newSelf

    def __mul__(self, other):
        newTerm = Term()
        newTerm = self.coef * other.coef
        newTerm = self.exp + other.exp
        return newTerm
    ...
    def __mul__(self, otherPoly): # mulPoly = myPoly * otherPoly
        ...
        for i in range(len(self)):
            for j in range(len(otherPoly)):
                sTerm = self.polyList[i]
                oTerm = otherPoly.polyList[i]
                result.addTerm(sTerm * oTerm)
        return result
